    ...
    def _connect_wpf_elements(self, names):
        ...
        for name in names:
            if getattr(self, name, None) is not None:
                raise Exception(
                    'Class ("{0}") already has a field/function called "{1}" or element name "{1}" is used twice in the xaml.'.format(
    ...
    def _prepare_xaml(self, xaml):
        ...
        for elem in tree.iter():
            ...
            events_on_element = [
                (
                    x,
                    get_from_any_ns(elem, x, nsmap),
                )
                for x in known_events
                if get_from_any_ns(elem, x, nsmap) is not None
            ]
